...
{
    return QCoreApplication::translate("hellar-core", psz).toStdString();
}
...
static void initTranslations(QTranslator &qtTranslatorBase, QTranslator &qtTranslator, QTranslator &translatorBase, QTranslator &translator)
{
    ...
    // Load e.g. bitcoin_de.qm (shortcut "de" needs to be defined in hellar.qrc)
    if (translatorBase.load(lang, ":/translations/"))
        QApplication::installTranslator(&translatorBase);

    // Load e.g. bitcoin_de_DE.qm (shortcut "de_DE" needs to be defined in hellar.qrc)
...
/** Class encapsulating Hellar Core startup and shutdown.
...
/** Main Hellar application object */
...
void BitcoinApplication::initializeResult(int retval)
{
    ...
    if(retval)
    {
        ...
        if(pwalletMain)
        {
            helModel = new HelModel(phelDB);            
            walletModel = new WalletModel(platformStyle, pwalletMain, optionsModel);
            
            window->setHelModel(helModel);
        ...
        // hellar: URIs or payment requests:
...
{
    QMessageBox::critical(0, "Runaway exception", BitcoinGUI::tr("A fatal error occurred. Hellar can no longer continue safely and will quit.") + QString("\n\n") + message);
    ::exit(EXIT_FAILURE);
}
...
int main(int argc, char *argv[])
{
    ...
    Q_INIT_RESOURCE(hellar);
    Q_INIT_RESOURCE(hellar_locale);
    ...
    /// 6. Determine availability of data directory and parse hellar.conf
    ...
    if(!masternodeConfig.read(strErr)) {
        QMessageBox::critical(0, QObject::tr("Hellar"),
    ...
    // hellar: links repeatedly have their payment requests routed to this process:
