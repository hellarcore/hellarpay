...
// Copyright (c) 2015-2017 The Hellar developers
...
{
    LogPrintf("HellarMiner -- started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("hellar-miner");
    ...
    try {
        ...
        while (true) {
            ...
            BlockAssembler *block = new BlockAssembler(chainparams);
            std::unique_ptr<CBlockTemplate> pblocktemplate(block->CreateNewBlock(coinbaseScript->reserveScript));
            if (!pblocktemplate.get())
            {
                LogPrintf("HellarMiner -- Keypool ran out, please call keypoolrefill before restarting the mining thread\n");
                return;
            }
            CBlock *pblock = &pblocktemplate->block;
            IncrementExtraNonce(pblock, pindexPrev, nExtraNonce);

            LogPrintf("HellarMiner -- Running miner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        LogPrintf("HellarMiner -- terminated\n");
        throw;
    }
    catch (const std::runtime_error &e)
    {
        LogPrintf("HellarMiner -- runtime error: %s\n", e.what());
        return;
    }
}
