...
#if defined(NDEBUG)
# error "Hellar cannot be compiled without assertions."
#endif
...
const std::string strMessageMagic = "Hellar Signed Message:\n";
                              ...
                              const CAmount& nAbsurdFee, std::vector<COutPoint>& coins_to_uncache, bool fDryRun)
{
    ...
    BOOST_FOREACH(const CTxIn &txin, tx.vin)
    {
        ...
        if (itConflicting != pool.mapNextTx.end())
        {
            ...
            // Transaction conflicts with mempool and RBF doesn't exist in Hellar
...
CAmount GetBlockSubsidy(int nPrevBits, int nPrevHeight, const Consensus::Params& consensusParams, bool fSuperblockPartOnly)
{
    ...
    if (nPrevHeight < consensusParams.RemoveDependencyRewardOnDiffHeight) {
    ...
    {
        // Remove subsidy dependency on difficulty, but increase basic value on 20%
        nSubsidyBase = 6;
    }
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck = false)
{
    ...
    // HELLAR : CHECK TRANSACTIONS FOR INSTANTSEND
        ...
        for (const auto& tx : block.vtx) {
            ...
            for (const auto& txin : tx->vin) {
                ...
                if (instantsend.GetLockedOutPointTxHash(txin.prevout, hashLocked) && hashLocked != tx->GetHash()) {
                    ...
                    return state.DoS(10, error("ConnectBlock(HEL): transaction %s conflicts with transaction lock %s", tx->GetHash().ToString(), hashLocked.ToString()),
                                     REJECT_INVALID, "conflict-tx-lock");
                }
            }
        }
    } else {
        LogPrintf("ConnectBlock(HEL): spork is off, skipping transaction locking checks\n");
    }

    // HEL : MODIFIED TO CHECK MASTERNODE PAYMENTS AND SUPERBLOCKS

    // TODO: resync data (both ways?) and try to reprocess this block later.
    CAmount blockReward = nFees + GetBlockSubsidy(pindex->pprev->nBits, pindex->pprev->nHeight, chainparams.GetConsensus());
    std::string strError = "";
    if (!IsBlockValueValid(block, pindex->nHeight, blockReward, strError)) {
        return state.DoS(0, error("ConnectBlock(HELLAR): %s", strError), REJECT_INVALID, "bad-cb-amount");
    }

    if (!IsBlockPayeeValid(*block.vtx[0], pindex->nHeight, blockReward)) {
        mapRejectedBlocks.insert(std::make_pair(block.GetHash(), GetTime()));
        return state.DoS(0, error("ConnectBlock(HELLAR): couldn't find masternode or superblock payments"),
