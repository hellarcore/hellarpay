...
const char * const BITCOIN_CONF_FILENAME = "hellar.conf";
const char * const BITCOIN_PID_FILENAME = "hellard.pid";
    ...
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            if (mapMultiArgs.count("-debug")) {
                ...
                // "hellar" is a composite category enabling all Hellar-related debug output
                if(ptrCategory->count(std::string("hellar"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Hellar
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Hellar
    // Mac: ~/Library/Application Support/Hellar
    // Unix: ~/.hellar
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Hellar";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Hellar";
#else
    // Unix
    return pathRet / ".hellar";
#endif
#endif
}
