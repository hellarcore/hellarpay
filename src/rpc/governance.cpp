...
UniValue gobject_prepare(const JSONRPCRequest& request)
{
    ...
    // This command is dangerous because it consumes 5 HEL irreversibly.
    // If params are lost, it's very hard to bruteforce them and yet
    // users ignore all instructions on hellarcentral etc. and do not save them...
...
void gobject_vote_conf_help()
{
    throw std::runtime_error(
                ...
                "Vote on a governance object by masternode configured in hellar.conf\n"
                "\nArguments:\n"
...
UniValue gobject_vote_conf(const JSONRPCRequest& request)
{
    ...
    if (!fMnFound) {
        ...
        resultsObj.push_back(Pair("hellar.conf", statusObj));
...
[[ noreturn ]] void gobject_help()
{
    throw std::runtime_error(
            ...
            "  vote-conf          - Vote on a governance object by masternode configured in hellar.conf\n"
...
{ //  category              name                      actor (function)         okSafe argNames
  //  --------------------- ------------------------  -----------------------  ------ ----------
    /* Hellar features */
    { "hellar",               "getgovernanceinfo",      &getgovernanceinfo,      true,  {} },
    { "hellar",               "getsuperblockbudget",    &getsuperblockbudget,    true,  {"index"} },
    { "hellar",               "gobject",                &gobject,                true,  {} },
    { "hellar",               "voteraw",                &voteraw,                true,  {} },
