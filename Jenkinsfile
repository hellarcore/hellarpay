...
// which are part of the Hellar repo will allow modification to the Jenkinsfile.
...
for(int i = 0; i < targets.size(); i++) {
  ...
  tasks["${target}"] = {
    node {
      ...
      withEnv(env) {
        def builderImageName="hellar-builder-${target}"
        ...
        builderImage.inside("-t") {
          ...
          sh "cp -ra ${pwd}/. /hellar-src/"

          // restore cache
          def hasCache = false
          try {
            copyArtifacts(projectName: "hellar-hellar/${BRANCH_NAME}", optional: true, selector: lastSuccessful(), filter: "ci-cache-${target}.tar.gz")
          } catch (Exception e) {
          }
          if (fileExists("ci-cache-${target}.tar.gz")) {
            hasCache = true
            echo "Using cache from hellar-hellar/${BRANCH_NAME}"
          } else {
            try {
              copyArtifacts(projectName: 'hellarpay-hellar/develop', optional: true, selector: lastSuccessful(), filter: "ci-cache-${target}.tar.gz");
            } catch (Exception e) {
            }
            if (fileExists("ci-cache-${target}.tar.gz")) {
              hasCache = true
              echo "Using cache from hellarpay-hellar/develop"
            }
          }

          if (hasCache) {
            sh "cd /hellar-src && tar xzf ${pwd}/ci-cache-${target}.tar.gz"
          } else {
            sh "mkdir -p /hellar-src/ci-cache-${target}"
          }

          stage("${target}/depends") {
            sh 'cd /hellar-src && ./ci/build_depends.sh'
          }
          stage("${target}/build") {
            sh 'cd /hellar-src && ./ci/build_src.sh'
          }
          stage("${target}/test") {
            sh 'cd /hellar-src && ./ci/test_unittests.sh'
          }
          stage("${target}/test") {
            sh 'cd /hellar-src && ./ci/test_integrationtests.sh'
          }

          // archive cache and copy it into the jenkins workspace
          sh "cd /hellar-src && tar czfv ci-cache-${target}.tar.gz ci-cache-${target} && cp ci-cache-${target}.tar.gz ${pwd}/"
        }
