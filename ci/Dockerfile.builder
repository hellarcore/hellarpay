...
# x11_gost_hash
RUN git clone https://github.com/Hellarpay/x11_gost_hash
RUN cd x11_gost_hash && python3 setup.py install
...
RUN mkdir /hellar-src && \
  mkdir -p /cache/ccache && \
  mkdir /cache/depends && \
  mkdir /cache/sdk-sources && \
  chown $USER_ID:$GROUP_ID /hellar-src && \
