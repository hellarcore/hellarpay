...
export BUILDER_IMAGE_NAME="hellar-builder-$BUILD_TARGET-$JOB_NUMBER"
...
if [ "$BUILD_TARGET" = "arm-linux" ]; then
  ...
  export CPPFLAGS="-DDEBUG_LOCKORDER -DENABLE_HELLAR_DEBUG"
  export PYZMQ=true
  export RUN_TESTS=true
  ...
  export BITCOIN_CONFIG="--enable-gui --enable-reduce-exports --disable-miner --without-printsupport"
